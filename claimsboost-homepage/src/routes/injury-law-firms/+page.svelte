<script>
	...
	// Track the last synced store location to detect header navigation changes
	let lastSyncedStoreLocation = $state(null);

	// Sync location input from store changes (initial load + header navigation)
	// Uses untrack() for state we don't want to trigger re-runs
	$effect(() => {
		if (!browser) return;

		// Read store values (these ARE tracked - we want to react to store changes)
		const storeCity = $searchLocation.city;
		const storeState = $searchLocation.state;
		const storeZip = $searchLocation.zipCode;
		const storeLat = $searchLocation.latitude;
		const storeLng = $searchLocation.longitude;
		const userCity = $location.city;
		const userState = $location.state;

		// Read local state with untrack to prevent loops
		const cleared = untrack(() => hasManuallyCleared);
		const editing = untrack(() => isUserEditing);
		const initialized = untrack(() => initialLoadComplete);
		const lastSynced = untrack(() => lastSyncedStoreLocation);
		const currentInputValue = untrack(() => locationValue);

		// Check if store has a valid location with coordinates
		if (storeCity && storeState) {
			const storeLocationKey = `${storeCity}, ${storeState}`;
			const newInputValue = storeZip || storeLocationKey;

			// PRIORITY: Header navigation with coordinates to a NEW location
			// This should ALWAYS work, even if user previously cleared the field
			if (storeLat && storeLng && lastSynced !== null && storeLocationKey !== lastSynced) {
				// Reset cleared flag since user explicitly selected a new location
				hasManuallyCleared = false;
				locationValue = newInputValue;
				lastSyncedStoreLocation = storeLocationKey;
				hasPerformedInitialSearch = true;
				performSearch();
				return;
			}

			// If user manually cleared or is editing, don't auto-sync
			if (cleared || editing) return;

			// Case 2: Initial load or simple sync
			if (!initialized || newInputValue !== currentInputValue) {
				locationValue = newInputValue;
				lastSyncedStoreLocation = storeLocationKey;
				if (!initialized) {
		...
		// If user manually cleared or is editing, don't fallback to user location
		if (cleared || editing) return;

		// Fallback to user location on initial load
		if (!initialized && userCity && userState) {
			locationValue = `${userCity}, ${userState}`;
			initialLoadComplete = true;
		}
	});
	...
	async function handleSearch(event) {
		...
		// Reset editing flags since user submitted the search
		hasManuallyCleared = false;
		isUserEditing = false;
	...
	function handleLocationClear() {
		...
		isUserEditing = false;
		locationValue = '';
		// Also clear the search location store to prevent it from repopulating
	...
	async function performSearch() {
		...
		// Track the searched location to detect header navigation changes
		if (displayedLocation.city && displayedLocation.state) {
			lastSyncedStoreLocation = `${displayedLocation.city}, ${displayedLocation.state}`;
		}
